...
namespace MyView {
	...
	Shader::Shader(const std::string& vertexSrc, const std::string& fragmentSrc)
	{
		...
		if (isCompiled == GL_FALSE)
		{
			...
			MV_CORE_ASSERT(false, "Vertex shader compilation failure!");
			return;
		}
		...
		if (isCompiled == GL_FALSE)
		{
			...
			MV_CORE_ERROR("{0}", infoLog.data());
			MV_CORE_ASSERT(false, "Fragment shader compilation failure!");
			return;
		}
		...
		if (isLinked == GL_FALSE)
		{
			...
			MV_CORE_ERROR("{0}", infoLog.data());
			MV_CORE_ASSERT(false, "Shader link failure!");
			return;
		}
